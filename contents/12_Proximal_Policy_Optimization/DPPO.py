"""
A simple version of OpenAI's Proximal Policy Optimization (PPO).
    ...
    def update(self):
        ...
        while not COORD.should_stop():
            if GLOBAL_EP < EP_MAX:
                ...
                adv = self.sess.run(
                    self.advantage, {self.tfs: s, self.tfdc_r: r})
                # update actor and critic in a update loop
                [self.sess.run(self.atrain_op, {
                               self.tfs: s, self.tfa: a, self.tfadv: adv}) for _ in range(UPDATE_STEP)]
...
class Worker(object):
    ...
    def work(self):
        ...
        while not COORD.should_stop():
            ...
            GLOBAL_EP += 1
            print('{0:.1f}%'.format(GLOBAL_EP/EP_MAX*100), '|W%i' %
